...
if (CLOUD_MODE && CLOUD_ACCESS_KEY) {
  localStorage.setItem('cloudAccessKey', CLOUD_ACCESS_KEY);
}
...
// SignalR connection for cloud mode
...
function connectCloud() {
  ...
  try {
    ...
    // Register handlers
    hubConnection.on("Welcome", (data) => {
      console.log("Cloud welcome:", data);
      myName = data.name;
      ...
      if (!data.roverOnline) {
        ...
        if (disconnectStatus) {
          disconnectStatus.textContent = "Rover is offline";
        }
      } else {
    ...
    hubConnection.on("RoverStatus", (data) => {
      if (data.online) {
        statusEl.textContent = "Cloud: connected";
        disconnectOverlay?.classList.remove('visible');
      } else {
        ...
        disconnectOverlay?.classList.add('visible');
        if (disconnectStatus) {
          disconnectStatus.textContent = "Rover went offline";
        }
        // Stop motors locally
        wifiState = "OFFLINE";
    ...
    hubConnection.on("RoleStatus", (data) => {
    ...
    hubConnection.on("RescanResult", (data) => {
      handleRescanResult(data.result);
    });

    hubConnection.on("WebRtcSignal", (signal) => {
      handleWebRtcSignal(signal);
    });

    hubConnection.onreconnecting((error) => {
      console.log("Cloud reconnecting:", error);
      statusEl.textContent = "Cloud: reconnecting...";
      disconnectOverlay?.classList.add('visible');
      if (disconnectStatus) {
        disconnectStatus.textContent = "Reconnecting...";
      }
    });

    hubConnection.onreconnected((connectionId) => {
      console.log("Cloud reconnected:", connectionId);
      statusEl.textContent = "Cloud: connected";
      reconnectAttempts = 0;
      disconnectOverlay?.classList.remove('visible');
    });
    ...
    hubConnection.onclose((error) => {
      console.log("Cloud connection closed:", error);
      ...
      disconnectOverlay?.classList.add('visible');
      if (disconnectStatus) {
        disconnectStatus.textContent = "Connection lost";
      }
    ...
    hubConnection.start()
      ...
      .catch((err) => {
        ...
        statusEl.textContent = "Cloud: failed";
        const delay = Math.min(500 * Math.pow(1.5, reconnectAttempts), MAX_RECONNECT_DELAY);
        reconnectAttempts++;
        reconnectTimer = setTimeout(connect, delay);
      });
...
function showCloudConfigModal() {
  ...
  overlay.innerHTML = `
    <div style="background: #161b22; border: 2px solid #30363d; border-radius: 12px; padding: 32px; max-width: 400px;">
      <h2 style="color: #f0f6fc; margin-bottom: 16px;">Cloud Configuration</h2>
      <p style="color: #8b949e; margin-bottom: 16px;">Enter cloud relay settings:</p>
      <div style="margin-bottom: 12px;">
        <label style="color: #8b949e; display: block; margin-bottom: 4px;">Relay URL:</label>
        <input type="text" id="cloudRelayInput" placeholder="https://your-relay.azurewebsites.net"
          value="${localStorage.getItem('cloudRelayUrl') || ''}"
          style="width: 100%; padding: 8px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #e6edf3;">
      </div>
      <div style="margin-bottom: 16px;">
        <label style="color: #8b949e; display: block; margin-bottom: 4px;">Access Key:</label>
        <input type="password" id="cloudKeyInput" placeholder="your-access-key"
          value="${localStorage.getItem('cloudAccessKey') || ''}"
          style="width: 100%; padding: 8px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #e6edf3;">
      </div>
      <button id="cloudConfigSave" style="background: #1f6feb; border: none; color: #fff; padding: 10px 20px; border-radius: 6px; cursor: pointer; width: 100%;">
        Connect
      </button>
    </div>
  `;
  ...
  document.getElementById('cloudConfigSave').addEventListener('click', () => {
    const url = document.getElementById('cloudRelayInput').value.trim();
    const key = document.getElementById('cloudKeyInput').value.trim();
    if (url && key) {
      localStorage.setItem('cloudRelayUrl', url);
      localStorage.setItem('cloudAccessKey', key);
      location.href = `?cloud=1&relay=${encodeURIComponent(url)}&key=${encodeURIComponent(key)}`;
    }
  });
}
...
function send(msg) {
  if (CLOUD_MODE) {
    // Cloud mode uses SignalR with structured messages
    ...
    if (msg.startsWith("M ")) {
      ...
      const on = msg.split(' ')[1] === '1';
      hubConnection.invoke("SendIrLed", on);
    } else if (msg === "RESCAN") {
      hubConnection.invoke("SendRescan");
    } else if (msg === "CLAIM") {
      hubConnection.invoke("Claim");
    } else if (msg === "RELEASE") {
      hubConnection.invoke("Release");
    } else if (msg.startsWith("VERSION:")) {
      // Version check not needed in cloud mode
    }
  } else {
